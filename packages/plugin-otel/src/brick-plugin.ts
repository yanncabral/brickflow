 ...
 * `./tracer.ts` (zero runtime dependencies). Use the `otel-adapter` subpath
 * to back it with a real OpenTelemetry tracer.
 *
 * The `BrickPlugin*` interfaces below structurally mirror
 * `brickflow` core's `src/plugin/types.ts` (separate workstream, not yet
 * merged), so the object returned by {@link createTracingPlugin} is directly
 * assignable to core's `BrickPlugin` without importing core:
 *
 * - `replay?: 'emit' | 'skip'` (default `'skip'`; core also enforces this).
 * - `onStart` / `onSuccess` (result) / `onFailure` (typed domain failure) /
 *   `onDefect` (unexpected throw) / `onSignal` / `onFinally` (terminal exit).
